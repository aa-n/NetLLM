...
PLM_DIR = (
    # "../../downloaded_plms"
    "../downloaded_plms"
    if "cluster_job_scheduling" in os.getcwd()
    else "../downloaded_plms"
)
PLM_FT_DIR = "artifacts/ft_plms"
RESULTS_DIR = (
    "artifacts/results/tpch"
    if "cluster_job_scheduling" not in os.getcwd()
    else "artifacts/results/tpch"
)
PLM_EMBED_SIZES = {
    "gpt2": {
        "base": 1024,
...
def load_model(args, model, model_dir):
    if args.rank > 0:
        ...
        model.modules_except_plm.load_state_dict(
            torch.load(os.path.join(model_dir, "modules_except_plm.bin"))
        )
    else:
...
def train(
    ...
    for iter in range(start_iter, args.num_iters):
        ...
        if iter % args.eval_per_iter == 0:
            eval_logs = evaluate_episode(
                args,
                env=eval_env,
                model=model,
                target_return=target_return,
                max_ep_len=min(args.eval_max_ep_len, max_ep_len),
...
def run(args):
    ...
    assert (
        args.exp_pool_path is not None
    ), "please specify a experience pool path for training"
    ...
    plm, *_ = load_plm(
        args.plm_type,
        os.path.join(PLM_DIR, "llama/base"),
    ...
    if args.pt_encoder_config is not None:
        ...
        assert (
            args.state_feature_dim is not None
        ), "please specify state feature dim if you dont use a pretrained encoder"
        state_encoder = EncoderNetwork(
            num_node_features=NUM_NODE_FEATURES,
            hidden_dim=args.state_feature_dim >> 1,
    ...
    max_ep_len = exp_dataset_info.max_timestep + 1
    rl_policy = OfflineRLPolicy(
        stage_state_dim=args.stage_embed_dim,
        exec_state_dim=args.exec_embed_dim,
        max_stage_num=args.max_stage_num,
        max_exec_num=args.max_exec_num,
    ...
    models_dir = os.path.join(
        ...
        f"peft_{args.rank}_K_{args.K}_gamma_{args.gamma}_sfd_{args.state_feature_dim}"
    ...
    result_path = os.path.join(
        results_dir,
        f"early_stop_{args.which_layer}_results_dt_{args.plm_type}_{args.plm_size}_peft_{args.rank}_K_{args.K}_gamma_{args.gamma}_tgt_scale_{args.target_return_scale}_seed_{args.seed}.pkl",
    )
    ...
    if args.train:
        ...
        console_log = open(
            os.path.join(models_dir, f"early_stop_{args.which_layer}_console.log"), "w"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--exp-pool-path",
        help="the path storing the experience pool file for training",
        default="artifacts/exp_pool/exp_pool.pkl",
    )
    parser.add_argument(
        "--sample-step", type=int, help="the steps for sampling experiences"
    )
    # environment settings
    parser.add_argument(
        "--num-executors",
        help="the total number of executors in the simulation",
        type=int,
        default=50,
    )
    parser.add_argument(
        "--job-arrival-cap",
        help="the total number of jobs that arrive throughout the simulation",
        type=int,
        default=200,
    )
    parser.add_argument(
        "--job-arrival-rate",
        help="non-negative number that controls how quickly new jobs arrive into the system.",
        type=float,
        default=4.0e-5,
    )
    parser.add_argument(
        "--moving-delay",
        help="time in ms it takes for a executor to move between jobs",
        type=float,
        default=2000.0,
    )
    parser.add_argument(
        "--warmup-delay",
        help="an executor is slower on its first task from  a stage if it was previously "
        "idle or moving jobs, which is caputred by adding a warmup delay to the task duration",
        type=float,
        default=1000.0,
    )
    parser.add_argument(
        "--render-mode",
        dest="render_mode",
        help='if set to "human", then a visualization of the simulation is rendred in real time',
        default=None,
    )
    parser.add_argument(
        "--dataset",
        help='choice of dataset to generate jobs from. Currently, only "tpch" is supported',
        default="tpch",
    )
    # plm settings
    ...
    parser.add_argument(
        "--rank",
        type=int,
        help="rank of low rank matrices training. if set to -1, low rank will not be enabled",
        default=128,
    )
    ...
    parser.add_argument(
        "--state-feature-dim", type=int, help="feature dim of the state encoder"
    )
    # offline policy model related settings
    parser.add_argument(
        "--K",
        type=int,
        help="horizon of offline policy to observe the past states, actions and returns (check the paper if you dont understand)",
        default=20,
    )
    parser.add_argument(
        "--gamma", type=float, help="discounted factor of reward", default=1.0
    )
    ...
    parser.add_argument(
        "--eval-max-ep-len",
        type=int,
        help="the max episode length of evaluation",
        default=6000,
    )
    parser.add_argument(
        "--eval-per-iter", type=int, help="evaluation per iteration", default=1
    )
    parser.add_argument(
        "--save-checkpoint-per-iter", type=int, help="saving checkpoint per iteration"
    )
    parser.add_argument(
        "--target-return-scale",
        type=float,
        help="target return, see the original paper for details",
        default=1.0,
    )
    parser.add_argument(
        "--which-layer",
        type=int,
        help="for early stopping: specify which layer to stop (layer index starts from 0)",
        default=-1,
    )
    # other settings
    ...
    parser.add_argument(
        "--grad-accum-steps", dest="grad_accum_steps", type=int, default=32
    )
    ...
    parser.add_argument(
        "--use-head",
        help="Use which head to predict next job stage",
        type=int,
        default=3,
    )
    parser.add_argument(

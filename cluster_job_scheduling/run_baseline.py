...
def test(env_settings, scheduler, seed, result_path):
    ...
    pickle.dump(
        {"job_durations": job_durations, "avg_job_duration": avg_job_duration},
        open(result_path, "wb"),
    )
...
def run(args):
    ...
    results_dir = os.path.join(
        ...
        "results",
        args.dataset,
        f"exe_{args.num_executors}_cap_{args.job_arrival_cap}_rate_{args.job_arrival_rate}_md_{args.moving_delay}_wd_{args.warmup_delay}_env_seed_{args.env_seed}",
    )
    ...
    result_path = os.path.join(
        results_dir,
        f"result_{args.sched}_seed_{args.seed}_{args.result_file_label}.pkl",
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--state-dict-path", help="the path of decima state dict", default=None
    )
    # other settings
    ...
    parser.add_argument(
        "--result-file-label",
        help="Label to distingush result file",
        type=str,
        default="pretrained",
    )

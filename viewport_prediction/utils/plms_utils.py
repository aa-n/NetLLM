...
def create_device_map_for_llama(
    device_input_side: str, device_output_side: str, device_middle_side: str = None
):
    """
    ...
    :param device_map
    """
    device_map = {"model.embed_tokens": device_input_side}
    ...
    for i in range(32):  # llama-7b has 32 transformer blocks
        device_map[f"model.layers.{i}"] = device_list[
            i // math.ceil(32 / len(device_list))
        ]
...
def load_plm(model_name, model_path, specials_to_add=None, **kwargs):
    ...
    """
    print("flag", "model_name:", model_path, model_name)
    ...
    if "mistral" in model_name:
        specials_to_add = ["<pad>"]
    if (
        "bert" in model_name and "roberta" not in model_name
    ):  # add is_decoder=True for BERT
    ...
    if (
        "llama" in model_name
        and device_input_side is not None
        and device_output_side is not None
    ):

...
def track_train(args, model, dataloader_train, dataloader_valid, models_dir):
    file_prefix = (
        f"his_{args.his_window}_fut_{args.fut_window}_ss_{args.sample_step}_"
    ...
    print(
        f"Training {args.model} on {args.train_dataset} - bs: {args.bs} - lr: {args.lr} - seed: {args.seed}"
    )
    ...
    for epoch in range(args.epochs):
        ...
        for batch, (r_his, r_fut, r_his_images, r_fut_images, _) in enumerate(
            dataloader_train
        ):
            gt = r_fut.to(args.device)
            r_his, r_fut = r_his.to(args.device), r_fut[:, 0:1, :].to(args.device)
            his, fut = normalize_data(r_his, args.train_dataset), normalize_data(
                r_fut, args.train_dataset
            )
            his_images = (
                torch.stack(r_his_images, dim=0)
                .permute(1, 0, 2, 3)
                .unsqueeze(-1)
                .to(args.device)
            )
            fut_images = (
                torch.stack(r_fut_images, dim=0)
                .permute(1, 0, 2, 3)
                .unsqueeze(-1)
                .to(args.device)
            )
            ...
            optimizer.step()
            print(
                f"\rTrain: [{batch + 1}/{train_size}] - train_loss: {train_loss:>9f}",
                end="",
            )
        ...
        if epoch % args.epochs_per_valid == 0:
            ...
            with torch.no_grad():
                ...
                for batch, (
                    ...
                    r_fut_images_val,
                    _,
                ) in enumerate(dataloader_valid):
                    gt_val = r_fut_val.to(args.device)
                    r_his_val, r_fut_val = r_his_val.to(args.device), r_fut_val[
                        :, 0:1, :
                    ].to(args.device)
                    his_val, fut_val = normalize_data(
                        r_his_val, args.train_dataset
                    ), normalize_data(r_fut_val, args.train_dataset)
                    his_images_val = (
                        torch.stack(r_his_images_val, dim=0)
                        .permute(1, 0, 2, 3)
                        .unsqueeze(-1)
                        .to(args.device)
                    )
                    fut_images_val = (
                        torch.stack(r_fut_images_val, dim=0)
                        .permute(1, 0, 2, 3)
                        .unsqueeze(-1)
                        .to(args.device)
                    )
                ...
                if best_valid_loss > mean_valid_loss:
                    ...
                    print(
                        f"Best model (epoch {best_epoch}, loss {best_valid_loss}) saved at",
                        best_model_path,
                    )
...
def test(args, model, dataloader_test, models_dir, results_dir):
    ...
    best_model_path = (
        os.path.join(models_dir, "best_model_" + file_prefix + ".pth")
    ...
    if args.model not in [
        "regression",
        "velocity",
    ]:  # linear regression/velocity doesn't need loading model weights
    ...
    with torch.no_grad():
        for raw_history, raw_future, info in tqdm(dataloader_test):
            raw_history, raw_future = raw_history.to(args.device), raw_future.to(
                args.device
            )
            history, future = normalize_data(
                raw_history, args.test_dataset
            ), normalize_data(raw_future, args.test_dataset)
            pred, gt = model.inference(history, future)
            pred, gt = denormalize_data(pred, args.test_dataset), denormalize_data(
                gt, args.test_dataset
            )
...
def track_test(args, model, dataloader_test, models_dir, results_dir):
    ...
    with torch.no_grad():
        for batch, (
            ...
            r_fut_images_test,
            info,
        ) in enumerate(dataloader_test):
            gt_test = r_fut_test.to(args.device)
            r_his_test, r_fut_test = r_his_test.to(args.device), r_fut_test[
                :, 0:1, :
            ].to(args.device)
            his_test, fut_test = normalize_data(
                r_his_test, args.train_dataset
            ), normalize_data(r_fut_test, args.train_dataset)
            his_images_test = (
                torch.stack(r_his_images_test, dim=0)
                .permute(1, 0, 2, 3)
                .unsqueeze(-1)
                .to(args.device)
            )
            fut_images_test = (
                torch.stack(r_fut_images_test, dim=0)
                .permute(1, 0, 2, 3)
                .unsqueeze(-1)
                .to(args.device)
            )
...
def run(args):
    ...
    models_dir = os.path.join(
        cfg.models_dir, args.model, args.train_dataset, f"{args.dataset_frequency}Hz"
    )
    results_dir = os.path.join(
        cfg.results_dir, args.model, args.test_dataset, f"{args.dataset_frequency}Hz"
    )
    ...
    model = create_model(
        args.model, args.his_window, args.fut_window, args.device, args.seed
    ).to(args.device)

    if args.compile:
        assert torch.__version__ >= "2.0.0", (
            "Compile model requires torch version >= 2.0.0, but current torch version is "
            + torch.__version__
        )
        print(
    ...
    if args.train:
        dataset_train, dataset_valid = create_dataset(
            args.train_dataset,
        ...
        dataloader_train = DataLoader(
            dataset_train, batch_size=args.bs, shuffle=True, pin_memory=True
        )
        dataloader_valid = DataLoader(
            dataset_valid, batch_size=args.bs, shuffle=False, pin_memory=True
        )
    ...
    if args.test:
        if args.model == "track":
            ...
            dataloader_test = DataLoader(
                dataset_test, batch_size=args.bs, shuffle=False, pin_memory=True
            )
            ...
            dataloader_test = DataLoader(
                dataset_test, batch_size=args.bs, shuffle=True, pin_memory=True
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model", action="store", dest="model", help="Model type, e.g., track."
    )
    parser.add_argument(
        "--compile",
        action="store_true",
    ...
    parser.add_argument(
        ...
        action="store",
        dest="his_window",
        help="(Optional) Historical window",
        type=int,
    )
    parser.add_argument(
        "--fut-window",
        action="store",
        dest="fut_window",
        help="(Optional) Future (prediction) window.",
        type=int,
    )
    parser.add_argument(
        "--trim-head",
        action="store",
        dest="trim_head",
        help="(Optional) Trim some part of the viewport trajectory head.",
        type=int,
    )
    parser.add_argument(
        "--trim-tail",
        action="store",
        dest="trim_tail",
        help="(Optional) Trim some part of the viewport trajectory tail.",
        type=int,
    )
    parser.add_argument(
        "--dataset-frequency",
        action="store",
        dest="dataset_frequency",
        help="(Optional) The frequency version of the dataset.",
        type=int,
    )
    parser.add_argument(
        "--sample-step",
        action="store",
        dest="sample_step",
        help="(Optional) The steps for sampling viewports.",
        type=int,
    )
    ...
    parser.add_argument(
        ...
        action="store",
        dest="epochs",
        help="(Optional) Neural network learning epochs.",
        type=int,
    )
    parser.add_argument(
        "--epochs-per-valid",
        action="store",
        dest="epochs_per_valid",
        type=int,
    ...
    parser.add_argument(
        ...
        action="store",
        dest="lr",
        help="(Optional) Neural network learning rate.",
        type=float,
    )
    parser.add_argument(
        "--weight-decay",
        action="store",
        dest="weight_decay",
        help="(Optional) Neural network weight decay.",
        type=float,
    )
    parser.add_argument(
        "--bs", action="store", dest="bs", help="(Optional) Batch size.", type=int
    )
    parser.add_argument(
        "--model-path",
        action="store",
    ...
    args.his_window = (
        cfg.default_history_window if args.his_window is None else args.his_window
    )
    args.fut_window = (
        cfg.default_future_window if args.fut_window is None else args.fut_window
    )
    ...
    args.epochs_per_valid = (
        cfg.default_epochs_per_valid
        if args.epochs_per_valid is None
        else args.epochs_per_valid
    )
    args.dataset_frequency = (
        cfg.default_dataset_frequency
        if args.dataset_frequency is None
        else args.dataset_frequency
    )
    args.sample_step = (
        cfg.default_sample_step if args.sample_step is None else args.sample_step
    )
    ...
    if args.model in ["regression", "velocity"]:
        ...
        args.device = "cpu"
        print(
            f"Detect model: {args.model}. Automatically disenable train and compile mode and set device to cpu."
        )
